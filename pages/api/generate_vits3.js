...
export default async function (req, res) {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      res.status(200).json({ prompt: userInput, result: resultString, wavUrl: url, repeat: repeat, id: id, frame: data.frame});
    } else {
      ...
      try {
        ...
        const data = {
            input: {
                ...
                sdp_ratio: vits_param[character]["sdp_ratio"],
                style_weight: vits_param[character]["style_weight"],
                text: audioString
            }
        }
        ...
        res.status(200).json({ prompt: userInput, result: resultString, audioContent: response.data.output.voice, wavUrl: "", audioString: audioString, hash: hashString, repeat: 1, id: id})
      } catch (error) {
        res.status(400).json({ prompt: userInput, result: "runpod_vits2 serverが起動していません"});
      }
    }
  } catch(error) {
